...
public class EdicaoViagem extends JFrame {
	 ...
	 * Create the frame.
	 */
	public EdicaoViagem(InfoViagem viagemSelecionada, ListViagens janela) {
		...
		JLabel lblNewLabel_3 = new JLabel("Id reserva da viagem:");
		lblNewLabel_3.setFont(new Font("Calibri", Font.BOLD, 12));
		contentPane.add(lblNewLabel_3, "cell 4 3");
		
		txtIdReserva = new JTextField();
		txtIdReserva.setText(String.valueOf(viagemSelecionada.getIdReserva()));
		contentPane.add(txtIdReserva, "cell 5 3 4 1,growx");
		txtIdReserva.setColumns(10);
		
		JLabel lblNewLabel_5 = new JLabel("Status da reserva:");
		lblNewLabel_5.setFont(new Font("Calibri", Font.BOLD, 12));
		contentPane.add(lblNewLabel_5, "cell 10 3");
		
		JComboBox <Status>cbStatus = new JComboBox<Status>();
		contentPane.add(cbStatus, "cell 11 3 6 1,growx");
		cbStatus.addItem(Status.SELECIONE);
		cbStatus.addItem(Status.RESERVADO);
		cbStatus.addItem(Status.ACONFIRMACAO);
		cbStatus.addItem(Status.CONFIRMADO);
		cbStatus.addItem(Status.PAGO);
		cbStatus.addItem(Status.NPAGO);
		cbStatus.setSelectedItem(String.valueOf(viagemSelecionada.getStatus()));
		...
		JLabel lblNewLabel_9_2 = new JLabel("Tipo de acomodação:");
		lblNewLabel_9_2.setFont(new Font("Calibri", Font.BOLD, 12));
		contentPane.add(lblNewLabel_9_2, "cell 4 8");
		
		JComboBox<Acomodacoes> cbAcomodacao = new JComboBox<Acomodacoes>();
		contentPane.add(cbAcomodacao, "cell 5 8 5 1,growx");
		cbAcomodacao.addItem(Acomodacoes.SELECIONE);
		cbAcomodacao.addItem(Acomodacoes.HOTEL);
		cbAcomodacao.addItem(Acomodacoes.POUSADA);
		cbAcomodacao.addItem(Acomodacoes.RESORT);
		cbAcomodacao.addItem(Acomodacoes.HOSTEL);
		cbAcomodacao.addItem(Acomodacoes.ALBERGUE);
		cbAcomodacao.setSelectedItem(String.valueOf(viagemSelecionada.getAcomodacao()));
		
		JLabel lblNewLabel_10 = new JLabel("Meio de transporte:");
		lblNewLabel_10.setFont(new Font("Calibri", Font.BOLD, 12));
		contentPane.add(lblNewLabel_10, "cell 10 8");
		
		JComboBox<Transportes> cbTransporte = new JComboBox<Transportes>();
		contentPane.add(cbTransporte, "cell 11 8 6 1,growx");
		cbTransporte.addItem(Transportes.SELECIONE);
		cbTransporte.addItem(Transportes.AVIAO);
		cbTransporte.addItem(Transportes.ALUGUELC);
		cbTransporte.addItem(Transportes.TRANP);
		cbTransporte.addItem(Transportes.BARCO);
		cbTransporte.addItem(Transportes.TRANSLADO);
		cbTransporte.addItem(Transportes.TAXI);
		cbTransporte.setSelectedItem(String.valueOf(viagemSelecionada.getTransporte()));
        ...
        txtIdReserva.setBorder(roundedBorder);
        ...
        cbStatus.setBorder(roundedBorder);
        cbAcomodacao.setBorder(roundedBorder);
        cbTransporte.setBorder(roundedBorder);
		...
		btnLimpar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				int selecione = 0;
				cbStatus.setSelectedIndex(selecione);
				cbAcomodacao.setSelectedIndex(selecione);
				cbTransporte.setSelectedIndex(selecione);
			}
		});
		...
		btnEditarViagem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				int idReservaInt=0;
				try {
					idReservaInt = Integer.parseInt(idReserva);
				}catch(Exception ex) {
					JOptionPane.showMessageDialog(null, "Id Reserva da viagem inválido. Digite apenas números!");
					return ;
					
				}
				viagemSelecionada.setIdReserva(idReservaInt);
				...
				if (cbAcomodacao.getSelectedItem()==Acomodacoes.SELECIONE) {
					JOptionPane.showMessageDialog(null, "Selecione uma opção de Acomodação");
					return ;
				}

				Acomodacoes acomSel = (Acomodacoes) cbAcomodacao.getSelectedItem();
				viagemSelecionada.setAcomodacao(acomSel);
				
				if (cbStatus.getSelectedItem()==Status.SELECIONE) {
					JOptionPane.showMessageDialog(null, "Selecione uma opção de Status da reserva");
					return ;
				} 
				
				Status staSel = (Status) cbStatus.getSelectedItem();
				viagemSelecionada.setStatus(staSel);
				
				if (cbTransporte.getSelectedItem()==Transportes.SELECIONE) {
					JOptionPane.showMessageDialog(null, "Selecione uma opção de Meio de transporte");
					return ;
				} 
				
				Transportes traSel = (Transportes) cbTransporte.getSelectedItem();
				viagemSelecionada.setTransporte(traSel);
